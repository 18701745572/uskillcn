...
/**
 * Write the uskillcn gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  ...
  // Packaged uskillcn loads the renderer from file://, so the gateway must allow
 ...
 * OpenClaw/uskillcn versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  ...
  // When uskillcn-managed moonshot provider exists, prefer centralized key
