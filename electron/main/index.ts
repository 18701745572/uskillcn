...
import { ensureuskillcnContext, repairuskillcnOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
// On Wayland this maps the running window to uskillcn.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('uskillcn.desktop');
}
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== uskillcn Application Starting ===');
  logger.debug(
  ...
  // Repair any bootstrap files that only contain uskillcn markers (no OpenClaw
  // template content). This fixes a race condition where ensureuskillcnContext()
  // previously created the file before the gateway could seed the full template.
  void repairuskillcnOnlyBootstrapFiles().catch((error) => {
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running') {
      void ensureuskillcnContext().catch((error) => {
        logger.warn('Failed to re-merge uskillcn context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge uskillcn context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureuskillcnContext will retry until the target files appear.
  void ensureuskillcnContext().catch((error) => {
    logger.warn('Failed to merge uskillcn context into workspace:', error);
  });
