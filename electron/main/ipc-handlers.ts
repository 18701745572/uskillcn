...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  // UI-created tasks have no delivery target — results go to the uskillcn chat page.
  ...
  ipcMain.handle('cron:create', async (_, input: {
    ...
    try {
      const gatewayInput = {
        ...
        wakeMode: 'next-heartbeat',
        sessionTarget: 'isolated',
        // UI-created jobs deliver results via uskillcn WebSocket chat events,
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        console.log(`[uskillcn-validate] validating provider type: ${providerType}`);
