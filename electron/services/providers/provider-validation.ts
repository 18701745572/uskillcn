...
function logValidationStatus(provider: string, status: number): void {
  console.log(`[uskillcn-validate] ${provider} HTTP ${status}`);
}
...
function logValidationRequest(
  ...
  console.log(
    `[uskillcn-validate] ${provider} request ${method} ${sanitizeValidationUrl(url)} headers=${JSON.stringify(sanitizeHeaders(headers))}`,
  );
}
...
async function validateOpenAiCompatibleKey(
  ...
  if (modelsResult.error?.includes('API error: 404')) {
    console.log(
      `[uskillcn-validate] ${providerType} /models returned 404, falling back to /chat/completions probe`,
    );
