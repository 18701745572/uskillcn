...
# Post-installation script for uskillcn on Linux
...
# Create symbolic link for uskillcn app binary
if [ -x /opt/uskillcn/uskillcn ]; then
    ln -sf /opt/uskillcn/uskillcn /usr/local/bin/uskillcn 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/uskillcn/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/uskillcn/chrome-sandbox' || true
else
    chmod 0755 '/opt/uskillcn/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/uskillcn/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/uskillcn'
...
echo "uskillcn has been installed successfully."
