; uskillcn Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  ; dialog during upgrades.  The built-in NSIS uninstaller deletes uskillcn.exe
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to completely remove all uskillcn user data?$\r$\n$\r$\nThis will delete:$\r$\n  • .openclaw folder (configuration & skills)$\r$\n  • AppData\Local\uskillcn (local app data)$\r$\n  • AppData\Roaming\uskillcn (roaming app data)$\r$\n$\r$\nSelect 'No' to keep your data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; --- Always remove current user's data first ---
    RMDir /r "$PROFILE\.openclaw"
    RMDir /r "$LOCALAPPDATA\uskillcn"
    RMDir /r "$APPDATA\uskillcn"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R2\.openclaw"
    RMDir /r "$R2\AppData\Local\uskillcn"
    RMDir /r "$R2\AppData\Roaming\uskillcn"
