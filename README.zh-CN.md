...
<p align="center">
  <a href="#功能特性">功能特性</a> •
  <a href="#为什么选择-uskillcn">为什么选择 uskillcn</a> •
  <a href="#快速上手">快速上手</a> •
  <a href="#系统架构">系统架构</a> •
  <a href="#开发指南">开发指南</a> •
  <a href="#参与贡献">参与贡献</a>
...
**uskillcn** 是连接强大 AI 智能体与普通用户之间的桥梁。基于 [OpenClaw](https://github.com/OpenClaw) 构建，它将命令行式的 AI 编排转变为易用、美观的桌面体验——无需使用终端。

无论是自动化工作流、连接通讯软件，还是调度智能定时任务，uskillcn 都能提供高效易用的图形界面，帮助你充分发挥 AI 智能体的能力。

uskillcn 预置了最佳实践的模型供应商配置，原生支持 Windows 平台以及多语言设置。当然，你也可以通过 **设置 → 高级 → 开发者模式** 来进行精细的高级配置。
...
构建 AI 智能体不应该需要精通命令行。uskillcn 的设计理念很简单：**强大的技术值得拥有一个尊重用户时间的界面。**
...
uskillcn 直接基于官方 **OpenClaw** 核心构建。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
### 🌙 自适应主题
支持浅色模式、深色模式或跟随系统主题。uskillcn 自动适应你的偏好设置。
...
从 [Releases](https://github.com/18701745572/uskillcn/releases) 页面下载适用于你平台的最新版本。
...
首次启动 uskillcn 时，**设置向导** 将引导你完成以下步骤：
...
> Moonshot（Kimi）说明：uskillcn 默认保持开启 Kimi 的 web search。  
> 当配置 Moonshot 后，uskillcn 也会将 OpenClaw 配置中的 Kimi web search 同步到中国区端点（`https://api.moonshot.cn/v1`）。

### 代理设置

uskillcn 内置了代理设置，适用于需要通过本地代理客户端访问外网的场景，包括 Electron 本身、OpenClaw Gateway，以及 Telegram 这类频道的联网请求。
...
- 如果启用了 Telegram，uskillcn 还会把代理同步到 OpenClaw 的 Telegram 频道配置中。

---

## 系统架构

uskillcn 采用 **双进程 + Host API 统一接入架构**。渲染进程只调用统一客户端抽象，协议选择与进程生命周期由 Electron 主进程统一管理：
...
我们欢迎社区的各种贡献！无论是修复 Bug、开发新功能、改进文档还是翻译——每一份贡献都让 uskillcn 变得更好。
...
我们正在启动 uskillcn 合作伙伴计划，寻找能够帮助我们将 uskillcn 介绍给更多客户的合作伙伴，尤其是那些有定制化 AI 智能体或自动化需求的客户。

合作伙伴负责帮助我们连接潜在用户和项目，uskillcn 团队则提供完整的技术支持、定制开发与集成服务。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/uskillcn&type=Date" alt="Stars 历史图表" />
</p>
...
uskillcn 基于 [MIT 许可证](LICENSE) 发布。你可以自由地使用、修改和分发本软件。
